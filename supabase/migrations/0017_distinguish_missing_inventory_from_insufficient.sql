-- =============================================================================
-- 0017_distinguish_missing_inventory_from_insufficient.sql —
-- diagnostic split in pos_record_sale_v2.
--
-- The 0011 / 0014 body collapsed two distinct error conditions into the
-- single `insufficient_stock` raise: (1) the inventory row exists but the
-- quantity is too low, and (2) no inventory row exists at all for the
-- product/warehouse pair. The collapsed UPDATE was:
--
--   UPDATE public.inventory SET quantity = quantity - n
...
--
-- Both "no matching row" and "matching row with quantity < n" produced
-- IF NOT FOUND, so the client received `insufficient_stock` even when the
-- product had never been stocked at this warehouse — a misleading
-- diagnostic.
--
-- Fix: lock the row with SELECT ... FOR UPDATE first (matching the wallet
-- pattern from 0014). If NOT FOUND, raise the new `inventory_row_missing`
-- error. Otherwise compare the locked quantity against the requested
-- amount; raise `insufficient_stock` (now with an `available_qty` hint) if
-- short, else perform the unguarded UPDATE on the locked row.
--
-- No client contract change. The new error code is additive — clients
-- that don't switch on error codes will still see a clear message and
-- the existing retry/abort behavior is unaffected.
--
-- Findings 1 & 2 (NULL stock_transaction_ids in pos_inventory_delta) are
-- already addressed in v2 by the `movement_id_required` validation at
-- 0011:400-401, so no change there.
--
-- Finding 4 (non-idempotent initial-stock in pos_create_product_v2) is
-- already addressed by the `IF v_inserted AND p_initial_stock IS NOT NULL`
-- guard at 0011:587 — a retry's products INSERT lands on ON CONFLICT DO
-- NOTHING, sets v_inserted=false, and the entire initial-stock block is
-- skipped.
-- =============================================================================
