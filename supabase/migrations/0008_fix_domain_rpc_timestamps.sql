-- =============================================================================
-- 0008_fix_domain_rpc_timestamps.sql
-- Follow-up to 0007: created_at/last_updated_at also need defaulting.
--
-- The client's Drift Companions sometimes stamp last_updated_at but never
-- set created_at — Drift's local table has withDefault(currentDateAndTime),
-- so locally the column populates, but serializeInsertable emits only the
-- present Companion fields. Cloud `created_at NOT NULL DEFAULT now()` then
-- crashes on `jsonb_populate_record(NULL::T, p)` for the same reason as
-- 0007. Fix the same way: inject now() into the populate call so the
-- column defaults when absent from the payload, while still letting any
-- client-stamped value win via JSONB right-merge.
-- =============================================================================
...
BEGIN
  ...
  SELECT (jsonb_populate_record(
    ...
    v_ts_defaults || c_products_defaults || p_product
  )).*
