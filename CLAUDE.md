...
6.  **Schema Migrations**: Every time `schemaVersion` is bumped, add a matching `if (from < newVersion)` block inside `onUpgrade` in `app_database.dart`. Use `m.addColumn(table, table.column)` for new columns and `m.createTable(table)` for new tables. Never drop tables in `onUpgrade` — that wipes all user data. The `createTable` loop at the top of `onUpgrade` handles brand-new tables automatically; only new columns on existing tables need an explicit `addColumn` step.
