...
class AppDatabase extends _$AppDatabase {
  ...
  MigrationStrategy get migration => MigrationStrategy(
    onCreate: (m) async {
      ...
      debugPrint('[AppDatabase] onCreate: Seeding essential auth data...');
      await _seedEssentialAuthData(); // warehouses, essential staff/users

      debugPrint(
        '[AppDatabase] onCreate: Seeding heavy business data...',
      );
      try {
        await _seedHeavyBusinessData(); // products, inventory, customers, extra staff
        debugPrint('[AppDatabase] Heavy business seeding complete.');
      } catch (e) {
        debugPrint('[AppDatabase] Heavy business seeding error: $e');
      }
  ...
  /// Seeds only the essential data required for a user to log in.
  /// (Manufacturers, Warehouses, Users). This keeps onCreate very fast.
  Future<void> _seedEssentialAuthData() => transaction(() async {
    // ── 0. Supplier ─────────────────────────────────────────────────────────
    ...
    // ── 1. Manufacturers ────────────────────────────────────────────────────
    await into(manufacturers).insert(
    ...
    // ── 2. Warehouses ───────────────────────────────────────────────────────
    final pankshinId = await into(
      warehouses,
    ).insert(const WarehousesCompanion(name: Value('Pankshin Branch')));
    final keffiId = await into(
      warehouses,
    ).insert(const WarehousesCompanion(name: Value('Keffi Branch')));
    final tafawaId = await into(
      warehouses,
    ).insert(const WarehousesCompanion(name: Value('Tafawa Balewa Branch')));
    ...
    await batch((b) {
      ...
      b.insert(
        users,
        UsersCompanion(
          name: const Value('Okwor Camillus'),
          role: const Value('CEO'),
          roleTier: const Value(5),
          pin: const Value('000000'),
          warehouseId: Value(pankshinId),
      ...
      b.insert(
        users,
        UsersCompanion(
          name: const Value('OkworFelister'),
          role: const Value('manager'),
          roleTier: const Value(4),
          pin: const Value('111100'),
          warehouseId: Value(pankshinId),
      ...
      b.insert(
        users,
        UsersCompanion(
          name: const Value('Okwor Solomon'),
          role: const Value('stock_keeper'),
          roleTier: const Value(3),
          pin: const Value('222200'),
          warehouseId: Value(pankshinId),
          avatarColor: const Value('#F59E0B'),
        ),
      );
      b.insert(
        users,
        UsersCompanion(
          name: const Value('Okwor Malachi'),
          role: const Value('stock_keeper'),
          roleTier: const Value(3),
          pin: const Value('333300'),
          warehouseId: Value(pankshinId),
          avatarColor: const Value('#F59E0B'),
        ),
      );
      b.insert(
        users,
        UsersCompanion(
          name: const Value('Boniface'),
          role: const Value('rider'),
          roleTier: const Value(1),
          pin: const Value('444400'),
          warehouseId: Value(pankshinId),
          avatarColor: const Value('#EF4444'),
        ),
      );
      // Keffi Branch
      b.insert(
        users,
        UsersCompanion(
          name: const Value('Okwor Chimezie'),
          role: const Value('manager'),
          roleTier: const Value(4),
          pin: const Value('555500'),
          warehouseId: Value(keffiId),
          avatarColor: const Value('#3B82F6'),
        ),
      );
      b.insert(
        users,
        UsersCompanion(
          name: const Value('Eze Ebuka'),
          role: const Value('stock_keeper'),
          roleTier: const Value(3),
          pin: const Value('666600'),
          warehouseId: Value(keffiId),
          avatarColor: const Value('#F59E0B'),
        ),
      );
      ...
      b.insert(
        users,
        UsersCompanion(
          name: const Value('Dashe Gwimyol'),
          role: const Value('manager'),
          roleTier: const Value(4),
          pin: const Value('777700'),
          warehouseId: Value(tafawaId),
          avatarColor: const Value('#3B82F6'),
        ),
      );
    });
  ...
  /// Seeds the rest of the business data. Run in the background via microtask
  /// to unblock the LoginScreen from showing 'Setting up'.
  Future<void> _seedHeavyBusinessData() => transaction(() async {
    // Re-resolve essential IDs internally since this runs detached!
    final coldcrateId = (await (select(
      suppliers,
    )..where((s) => s.name.equals('Coldcrate Ltd'))).getSingle()).id;
    final nbId = (await (select(
      manufacturers,
    )..where((m) => m.name.equals('Nigerian Breweries'))).getSingle()).id;
    final gnId = (await (select(
      manufacturers,
    )..where((m) => m.name.equals('Guinness Nigeria'))).getSingle()).id;

    final pankshinId = (await (select(
      warehouses,
    )..where((w) => w.name.equals('Pankshin Branch'))).getSingle()).id;
    final keffiId = (await (select(
      warehouses,
    )..where((w) => w.name.equals('Keffi Branch'))).getSingle()).id;
    final tafawaId = (await (select(
      warehouses,
    )..where((w) => w.name.equals('Tafawa Balewa Branch'))).getSingle()).id;
    ...
    // 3 glass products
    final starId = await into(products).insert(
      ProductsCompanion(
        name: const Value('Star Lager Beer'),
        categoryId: Value(glassCatId),
        manufacturerId: Value(nbId),
        manufacturer: const Value('Nigerian Breweries'),
        supplierId: Value(coldcrateId),
        crateGroupId: Value(bigCrateId),
        crateSize: const Value('big'),
        retailPriceKobo: const Value(1160000),
        sellingPriceKobo: const Value(1160000),
        buyingPriceKobo: const Value(1080000),
        unit: const Value('Bottle'),
        colorHex: const Value('#F59E0B'),
      ),
    );
    final heinId = await into(products).insert(
      ProductsCompanion(
        ...
        crateGroupId: Value(bigCrateId),
        crateSize: const Value('big'),
        retailPriceKobo: const Value(1350000),
        sellingPriceKobo: const Value(1350000),
        buyingPriceKobo: const Value(1200000),
        unit: const Value('Bottle'),
        colorHex: const Value('#10B981'),
      ),
    );
    final guinnId = await into(products).insert(
      ProductsCompanion(
        name: const Value('Guinness Stout'),
        categoryId: Value(glassCatId),
        manufacturerId: Value(gnId),
        manufacturer: const Value('Guinness Nigeria'),
        supplierId: Value(coldcrateId),
        crateGroupId: Value(bigCrateId),
        crateSize: const Value('big'),
        retailPriceKobo: const Value(1750000),
        sellingPriceKobo: const Value(1750000),
        buyingPriceKobo: const Value(1623000),
        unit: const Value('Bottle'),
        colorHex: const Value('#6B7280'),
      ),
    );
    // 2 non-glass products
    final maltaId = await into(products).insert(
      ProductsCompanion(
        ...
        manufacturer: const Value('Guinness Nigeria'),
        supplierId: Value(coldcrateId),
        retailPriceKobo: const Value(1435000),
        sellingPriceKobo: const Value(1435000),
        buyingPriceKobo: const Value(1350000),
        unit: const Value('Can'),
        colorHex: const Value('#92400E'),
      ),
    );
    final himaltId = await into(products).insert(
      ProductsCompanion(
        ...
        manufacturer: const Value('Nigerian Breweries'),
        supplierId: Value(coldcrateId),
        retailPriceKobo: const Value(1130000),
        sellingPriceKobo: const Value(1130000),
        buyingPriceKobo: const Value(1030000),
        unit: const Value('Can'),
    ...
    await batch((b) {
      ...
      b.insert(
        users,
        UsersCompanion(
          name: const Value('Usman Garba'),
          role: const Value('rider'),
          roleTier: const Value(1),
          pin: const Value('888800'),
          warehouseId: Value(pankshinId),
          avatarColor: const Value('#F97316'),
        ),
      );
      b.insert(
        users,
        UsersCompanion(
          name: const Value('Daniel Markus'),
          role: const Value('rider'),
          roleTier: const Value(1),
          pin: const Value('999900'),
          warehouseId: Value(keffiId),
          avatarColor: const Value('#F97316'),
        ),
      );
      b.insert(
        users,
        UsersCompanion(
          name: const Value('John Pwajok'),
          role: const Value('rider'),
          roleTier: const Value(1),
          pin: const Value('001100'),
          warehouseId: Value(tafawaId),
          avatarColor: const Value('#F97316'),
        ),
      );
      ...
      b.insert(
        users,
        UsersCompanion(
          name: const Value('Amina Yusuf'),
          role: const Value('cashier'),
          roleTier: const Value(2),
          pin: const Value('223300'),
          warehouseId: Value(pankshinId),
          avatarColor: const Value('#3B82F6'),
        ),
      );
      b.insert(
        users,
        UsersCompanion(
          name: const Value('Grace Dung'),
          role: const Value('cashier'),
          roleTier: const Value(2),
          pin: const Value('334400'),
          warehouseId: Value(pankshinId),
          avatarColor: const Value('#3B82F6'),
        ),
      );
      b.insert(
        users,
        UsersCompanion(
          name: const Value('Joseph Danjuma'),
          role: const Value('cashier'),
          roleTier: const Value(2),
          pin: const Value('445500'),
          warehouseId: Value(pankshinId),
          avatarColor: const Value('#3B82F6'),
        ),
      );
      b.insert(
        users,
        UsersCompanion(
          name: const Value('Fatima Idris'),
          role: const Value('cashier'),
          roleTier: const Value(2),
          pin: const Value('556600'),
          warehouseId: Value(keffiId),
          avatarColor: const Value('#3B82F6'),
        ),
      );
      b.insert(
        users,
        UsersCompanion(
          name: const Value('Emmanuel Lot'),
          role: const Value('cashier'),
          roleTier: const Value(2),
          pin: const Value('667700'),
          warehouseId: Value(keffiId),
          avatarColor: const Value('#3B82F6'),
        ),
      );
      b.insert(
        users,
        UsersCompanion(
          name: const Value('Mary Fom'),
          role: const Value('cashier'),
          roleTier: const Value(2),
          pin: const Value('778800'),
          warehouseId: Value(tafawaId),
          avatarColor: const Value('#3B82F6'),
        ),
      );
      ...
      b.insert(
        users,
        UsersCompanion(
          name: const Value('Haruna Bulus'),
          role: const Value('cleaner'),
          roleTier: const Value(1),
          pin: const Value('889900'),
          warehouseId: Value(pankshinId),
          avatarColor: const Value('#94A3B8'),
        ),
      );
      b.insert(
        users,
        UsersCompanion(
          name: const Value('Rebecca Luka'),
          role: const Value('cleaner'),
          roleTier: const Value(1),
          pin: const Value('990000'),
          warehouseId: Value(keffiId),
          avatarColor: const Value('#94A3B8'),
        ),
      );
      b.insert(
        users,
        UsersCompanion(
          name: const Value('Sunday Nden'),
          role: const Value('cleaner'),
          roleTier: const Value(1),
          pin: const Value('101000'),
          warehouseId: Value(tafawaId),
          avatarColor: const Value('#94A3B8'),
        ),
      );
    ...
    final c1 = await into(customers).insert(
      CustomersCompanion(
        name: const Value('Adamu Musa'),
        phone: const Value('08031234567'),
        address: const Value('Pankshin Market'),
        customerGroup: const Value('retailer'),
        warehouseId: Value(pankshinId),
      ),
    );
    await into(customerWallets).insert(
      CustomerWalletsCompanion.insert(walletId: 'wlt-seed-$c1', customerId: c1),
    );

    final c2 = await into(customers).insert(
      CustomersCompanion(
        name: const Value('Hadiza Bello'),
        phone: const Value('08056781234'),
        address: const Value('Pankshin Junction'),
        customerGroup: const Value('wholesaler'),
        warehouseId: Value(pankshinId),
      ),
    );
    await into(customerWallets).insert(
      CustomerWalletsCompanion.insert(walletId: 'wlt-seed-$c2', customerId: c2),
    );

    final c3 = await into(customers).insert(
      CustomersCompanion(
        name: const Value('Ibrahim Suleiman'),
        phone: const Value('08098765432'),
        address: const Value('Keffi Road'),
        customerGroup: const Value('retailer'),
        warehouseId: Value(keffiId),
      ),
    );
    await into(customerWallets).insert(
      CustomerWalletsCompanion.insert(walletId: 'wlt-seed-$c3', customerId: c3),
    );

    final c4 = await into(customers).insert(
      CustomersCompanion(
        name: const Value('Ngozi Okafor'),
        phone: const Value('07061234567'),
        address: const Value('Keffi Market'),
        customerGroup: const Value('wholesaler'),
        warehouseId: Value(keffiId),
      ),
    );
    await into(customerWallets).insert(
      CustomerWalletsCompanion.insert(walletId: 'wlt-seed-$c4', customerId: c4),
    );

    final c5 = await into(customers).insert(
      CustomersCompanion(
        name: const Value('Yusuf Danladi'),
        phone: const Value('08123456789'),
        address: const Value('Tafawa Balewa Central'),
        customerGroup: const Value('retailer'),
        warehouseId: Value(tafawaId),
      ),
    );
    await into(customerWallets).insert(
      CustomerWalletsCompanion.insert(walletId: 'wlt-seed-$c5', customerId: c5),
    );
  });
